...
v0.1.0 では次の保守的ルールを追加しています。

- `生年月日` / `出生年月日` / `性別` / `続柄` / `国籍` / `電話番号` / `携帯番号` / `メールアドレス` などのラベル付き個人属性を `[個人属性]` として置換
- 姓名辞書を使った行政文脈限定氏名検出（`氏名より`, `氏名から`, `氏名について`, `氏名に係る`, `氏名（...）`, `氏名 様`, `氏名さん`）
- 番地直後に続く建物名・号室・階を含めた住所補完（例: `グリーンハイツ501号室`）

過検出抑制のため、自由文全体からの氏名抽出は避け、文脈限定を優先しています。
